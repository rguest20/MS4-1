...
def dashboard(request):
    ...
    try:
        ...
        messages.error(
            request,
            'There has been an error.  Please wait,\
            our admins will assign you soon.')
...
def dashboard_admin(request):
    ...
    return render(request, 'tickets/admindashboard.html',
                  {'unresolved': unresolved, 'weekold': weekold})
...
def admin_tickets(request):
    ...
    tickets = Ticket.objects.filter(
        resolved=False).all().order_by('-date_created')
...
def admin_tickets_single(request, number):
    ...
    comments = Comment.objects.filter(
        ticket=number).all().order_by('-date_sent')
    return render(request, 'tickets/admin/ticket-single.html',
...
def support(request):
    ...
    return render(request, 'tickets/support.html',
                  {'company': company, 'tickets': tickets})
...
def editticket(request, number):
    ...
    form = NewTicketForm({'subject': ticket.title,
                          'priority': ticket.severity,
                          'description': ticket.issue,
                          'category': 'Normal',
                          })
    return render(request, 'tickets/support/edit.html',
                  {'company': company, 'form': form, 'ticket': ticket})
...
def comment(request, number):
    ...
    company = clients.models.Client.objects.filter(
    ...
    form = CommentForm({'commmenter': company.client_name,
                       'comment': '', 'file': '', })
    comments = Comment.objects.filter(ticket=number).all()
    return render(request,
                  'tickets/comment.html',
                  {'company': company,
                   'form': form,
                   'ticket': ticket,
                   'comments': comments,

...
class NewTicketForm(forms.Form):
    subject = forms.CharField(label='Subject', max_length=100, validators=[MinLengthValidator(1, message="This field cannot be empty")])
    priority = forms.CharField(label='Priority', widget=forms.Select(choices=[('ST', 'Normal'),('HI', 'High'),('VH', 'Very High'), ('CR', 'Critical')]))
    description = forms.CharField(widget=TinyMCE(attrs={'cols': 80, 'rows': 30}), validators=[MinLengthValidator(1, message="This field cannot be empty")])
...
class CommentForm(forms.Form):
    ...
    comment = forms.CharField(widget=TinyMCE(attrs={'cols': 80, 'rows': 30}), required = True)
